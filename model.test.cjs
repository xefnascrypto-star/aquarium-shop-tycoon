...
let state={},l=M.create();assert.equal(l.rooms[0].width,16);assert.equal(l.objects.filter(o=>M.owned(o,state)&&['betta','comet','tank3'].includes(o.kind)).length,2);
...
const expanded=M.migrate(l,{shelf:true,expansion:true});assert.deepEqual([expanded.rooms[0].width,expanded.rooms[0].depth],[20,17]);assert.deepEqual(M.migrate(expanded,{shelf:true,expansion:true}),expanded);
