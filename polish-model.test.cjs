...
for(const p of [{x:-.1,y:3},{x:14.1,y:3},{x:3,y:-.1},{x:3,y:13.1}])assert.equal(N.clear(g,p),false);
...
console.log('PASS: +26% area, v0.6 position migration, three layouts/rotations, blocked and reopened passage, physical interaction cells, walls, radius clearance, rounded turns, continuous movement, front-of-tank depth. Samples:',samples);
