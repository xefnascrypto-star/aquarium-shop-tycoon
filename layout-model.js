...
(function(root,factory){const m=factory();if(typeof module==='object'&&module.exports)module.exports=m;else root.ShopLayout=m})(globalThis,()=>{
 ...
 function create(){return {version:2,rooms:[{id:'main',type:'retail',width:12,depth:12,origin:{x:500,y:185},entrance:{x:10,y:0},reserved:[{x:10,y:0,width:2,depth:2,label:'Entrada'}]}],objects:defaults.map(([id,kind,x,y])=>({id,kind,roomId:'main',x,y,rotation:0,placed:!catalog[kind].unlock}))}}
 ...
 if(saved&&[1,2].includes(saved.version)&&Array.isArray(saved.objects)){
 const accepted=[];
 ...
 return {catalog,roomTypes,create,clone,purchased,owned,footprint,validate,findFree,migrate,project,unproject};
});
