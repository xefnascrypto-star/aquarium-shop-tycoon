...
(function(root,factory){const m=factory(typeof module==='object'&&module.exports?require('./design.js'):root.ShopDesign);if(typeof module==='object'&&module.exports)module.exports=m;else root.ShopLayout=m})(globalThis,D=>{
 const catalog={betta:{label:'Acuario de Bettas',width:5,depth:3,action:'betta'},comet:{label:'Acuario de Cometas',width:5,depth:3,action:'comet'},tank3:{label:'Acuario de 60 L',width:5,depth:3,action:'tank3',unlock:'tank3'},shelf:{label:'Estantería',width:4,depth:2,action:'shelf',unlock:'shelf'},counter:{label:'Mostrador',width:6,depth:3,action:'counter'},plant:{label:'Planta decorativa',width:2,depth:2,action:'decoration'},warehouse:{label:'Recepción y almacén',width:2,depth:2,action:'warehouse'}};
 ...
 const defaults=[['betta-1','betta',0,2],['comet-1','comet',0,7],['shelf-1','shelf',6,0],['counter-1','counter',7,10],['plant-1','plant',0,0],['plant-2','plant',13,2],['plant-3','plant',0,10],['warehouse-1','warehouse',13,6],['tank-3','tank3',7,3],['tank-4','tank4',12,2],['tank-5','tank5',12,6],['shelf-2','shelf2',5,13],['battery-1','battery',1,12],['plants-1','plants',12,10],['professional-1','professional',6,12]];
 const clone=v=>JSON.parse(JSON.stringify(v));
 function dimensions(state={}){const base=state.expansion?D.space.expanded:D.space.initial;return {width:base.width+(state.microWidth?1:0),depth:base.depth+(state.microDepth?1:0)}}
 function create(){return {version:3,rooms:[{id:'main',type:'retail',width:D.space.initial.width,depth:D.space.initial.depth,origin:{x:500,y:185},entrance:{x:10,y:0},reserved:[{x:10,y:0,width:2,depth:2,label:'Entrada'}]}],objects:defaults.map(([id,kind,x,y])=>({id,kind,roomId:'main',x,y,rotation:0,placed:!catalog[kind].unlock}))}}
 ...
 const result=create();Object.assign(result.rooms[0],dimensions(state));
 ...
 return {dimensions,catalog,roomTypes,create,clone,purchased,owned,footprint,barriers,validate,findFree,migrate,project,unproject};
});
