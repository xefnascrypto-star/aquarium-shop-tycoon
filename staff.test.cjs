...
await p.screenshot({path:__dirname+'/v09-'+name+'.png',fullPage:true});
}
...
await p.screenshot({path:__dirname+'/v09-'+phase+'.png',fullPage:true});
...
assert.deepEqual(errors,[]);fs.writeFileSync(__dirname+'/staff-v09-report.json',JSON.stringify({layouts:reports,persistedPhases:['preparing-fish','carrying-order','checkout'],supplierOrdersPreserved:true,staffOnlyBlockedAccess:true,errors},null,2));console.log('PASS: physical worker paths, one/two staff, concurrent jobs, three layouts, preparation/bags/payment, work-side warnings, task and supplier persistence.');
