...
await page.clock.install({time:new Date('2026-09-07T12:00:00')});await page.goto('http://127.0.0.1:4173');
...
await page.locator('[data-panel="stock"]').click();assert.equal(await page.locator('#panel').isVisible(),true);await page.keyboard.press('Escape');assert.equal(await page.locator('#panel').isVisible(),false);
