...
assert.equal(await p.evaluate(()=>sellBasket({betta:1})),null);assert.ok(await p.evaluate(()=>(()=>{state.shelf=true;state.layout.objects.find(o=>o.kind==='shelf').placed=true;state.logistics=null;ShopLogistics.ensure();return sellBasket(ShopDesign.kit)})()));assert.equal(await p.evaluate(()=>state.stock.betta),2);assert.ok(await p.evaluate(()=>sellBasket({betta:2},{requestId:'coexist'})));assert.equal(await p.evaluate(()=>state.moments.request),null);assert.equal(await p.evaluate(()=>sellBasket({betta:2},{requestId:'coexist'})),null);
