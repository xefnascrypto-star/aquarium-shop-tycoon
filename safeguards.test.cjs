...
await p.evaluate(()=>{state.level=7;state.money=1000;state.supplier='wholesale';state.orders=[];render()});
await p.evaluate(()=>order('neon',5));assert.equal(await p.evaluate(()=>incomingOrders().length),0);await p.evaluate(()=>order('neon',10));assert.equal(await p.evaluate(()=>state.money),890);assert.equal(await p.evaluate(()=>incomingOrders()[0].remaining),90000);
