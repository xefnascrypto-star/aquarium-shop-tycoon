...
const p=await browser.newPage({viewport:{width:390,height:844}}),errors=[],reports=[];p.on('pageerror',e=>errors.push(e.message));await p.clock.install({time:new Date('2026-09-09T12:00:00Z')});await p.clock.pauseAt(new Date('2026-09-09T12:00:01Z'));await p.goto('http://127.0.0.1:4174');
await p.evaluate(()=>{
...
await p.screenshot({path:__dirname+'/v07-'+name+'.png',fullPage:true});reports.push({name,updates:report.updates,sales:report.served,visited:report.visited});console.log('LAYOUT',JSON.stringify(reports.at(-1)));
}
...
assert.deepEqual(errors,[]);fs.writeFileSync(__dirname+'/polish-v07-report.json',JSON.stringify({layouts:reports,checks:'Every rendered frame: radius clearance, speed bound, product interaction and front occlusion. Three layouts, blocked/reopened route, save migration and mobile editor. PASS'},null,2));
await browser.close();
