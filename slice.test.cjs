...
for(const width of [320,390,768,1280]){await page.setViewportSize({width,height:960});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:__dirname+'/v010-finished-'+width+'.png',fullPage:true})}
assert.deepEqual(errors,[]);fs.writeFileSync(__dirname+'/playthrough-v010.json',JSON.stringify({levels:report,final:{money:final.money,served:final.served,level:final.level,xp:final.xp,pearls:final.pearls,kits:final.kits,groupSales:final.groupSales,employee:final.employee,wages:final.wagesPaid,lost:final.lost,investment:final.investment},checks:'Physical paths, pending cancel/reload, placement confirmation, all ten levels without money/XP cheats, suppliers, kit, groups, wages, first investment, persistence, responsive. PASS'},null,2));
