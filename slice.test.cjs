...
for(let step=0;step<2000;step++){
 ...
 if(info.level!==previous){levels.push(info.level);report.push({step,level:info.level,money:info.money,served:info.served,xp:info.xp});console.log('LEVEL',JSON.stringify(report.at(-1)));previous=info.level;await page.screenshot({path:__dirname+'/v06-level-'+info.level+'.png',fullPage:true})}
 if(info.level===10){assert.ok(info.money>=12000&&info.money<17000);full=true;break}
...
for(const width of [320,390,768,1280]){await page.setViewportSize({width,height:960});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:__dirname+'/v06-finished-'+width+'.png',fullPage:true})}
assert.deepEqual(errors,[]);fs.writeFileSync(__dirname+'/playthrough-v06.json',JSON.stringify({levels:report,final:{money:final.money,served:final.served,level:final.level,xp:final.xp,pearls:final.pearls,kits:final.kits,groupSales:final.groupSales,employee:final.employee,wages:final.wagesPaid,lost:final.lost,investment:final.investment},checks:'Physical paths, pending cancel/reload, placement confirmation, all ten levels without money/XP cheats, suppliers, kit, groups, wages, first investment, persistence, responsive. PASS'},null,2));
