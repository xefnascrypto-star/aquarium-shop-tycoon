...
function productRows(keys,ordering=false){return keys.map(k=>{const p=products[k],q=quote(k,orderQuantity),can=state.level>=2&&!state.delivery&&orderQuantity>=q.minimum&&state.money>=q.cost&&used()+p.vol*orderQuantity<=state.capacity;return '<div class="product-row"><span class="product-icon">'+p.icon+'</span><div><b>'+p.name+'</b><small>'+state.stock[k]+' disponibles'+(state.kitRequested&&ShopDesign.kit[k]?' · 1 reservado':'')+' · venta '+p.sell+' 🪙</small></div>'+(ordering?'<button data-order="'+k+'" '+(!can?'disabled':'')+'>'+orderQuantity+' × · '+fmt(q.cost)+' 🪙</button>':'')+'</div>'}).join('')}
function objectInfo(){
...
function syncScene(){
$('gameSpeed').value=state.speed;$('gameHint').textContent='Toca tu tienda · v0.6 · ritmo ×'+state.speed;$('used').textContent=used();$('capacity').textContent=state.capacity;
